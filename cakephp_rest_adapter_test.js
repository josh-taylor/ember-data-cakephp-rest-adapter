...
module("CakePHP REST Adapter", {
    setup: function() {
        ajaxUrl = undefined;
        ajaxType = undefined;
        ajaxHash = undefined;

        Adapter = DS.CakeRESTAdapter.extend();
...
test("creating a person makes a POST to /people/, with the data hash", function() {
    ...
    expectType("POST");
    expectData({ Person: { name: "John Doe", group: null } });
    
    // setup
    ajaxHash.success({ Person: { id: 1, name: "John Doe" } });
...
test("updating a person makes a PUT to /people/:id/ with the data hash", function() {
    ...
    expectType("PUT");
    expectData({ Person: { name: "Jane Doe", group: null } });

    // setup
    ajaxHash.success({ Person: { id: 1, name: "Jane Doe" } });
...
test("updating a group makes a PUT to /groups/:id/ with the data hash", function() {
    ...
    expectType("PUT");
    expectData({ Group: { name: "New Doe Family Name", zpeople: [1,2] } });

    // setup
    ajaxHash.success({ Group: { id: 1, name: "New Doe Family Name" } });
